...
const workbook = xlsx.readFile("databaseNOE.xlsx");
        ...
        fs.readdir(folderToBeScrapped, (err, files) => {
            ...
            for (i = 0; i < profilesToBeScraped.length; i++) {
                ...
                fs.readFile(folderToBeScrapped + currentProfile, "utf8", (err, html) => {
                    ...
                    // obtain URL
                    rawURL = $(".ember-view.link-without-visited-state.cursor-pointer.text-heading-small.inline-block.break-words").attr("href");
                    try {url = rawURL.split("detail")[0];} catch (error) {1+1}
                   ...
                   */
                    ...
                    xlsx.writeFile(workbook, "databaseNOE.xlsx");
                });
            }
        });
    }
